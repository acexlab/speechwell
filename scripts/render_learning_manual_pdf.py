...
DEFAULT_INPUT_MD = ROOT / "PROJECT_LEARNING_MANUAL.md"
DEFAULT_OUTPUT_PDF = ROOT / "PROJECT_LEARNING_MANUAL.pdf"
...
def build_styles() -> dict[str, ParagraphStyle]:
    ...
    return {
        ...
        "h3": ParagraphStyle(
            ...
            spaceBefore=6,
            spaceAfter=4,
            textColor=colors.HexColor("#3C5D7F"),
        ),
        "body": ParagraphStyle(
            "ManualBody",
...
def flush_list(items: list[str], story: list, styles: dict[str, ParagraphStyle]) -> None:
    if not items:
        return
    story.append(
        ListFlowable(
...
def main() -> None:
    input_md = Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else DEFAULT_INPUT_MD
    output_pdf = Path(sys.argv[2]).resolve() if len(sys.argv) > 2 else DEFAULT_OUTPUT_PDF
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=16 * mm,
        bottomMargin=14 * mm,
        title="SpeechWell Project Learning Manual",
        author="OpenAI Codex",
    )
